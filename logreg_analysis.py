...
def logreg_callback(prior, l, dimred, dataset_type):
    model = LogisticRegression.LogisticRegression(l, prior)
...
def main():
    ...
    l = [("$10^{-1}$", 0.1), ("$10^{-2}$", 0.01), ("$10^{-3}$", 0.001), ("$10^{-4}$", 0.0001), ("$10^{-5}$", 0.00001)]
    dataset_types = [("RAW", None), ("Z-Norm", "Z-Norm")]
    dimred = [("No PCA", None), ("PCA $(m=11)$", 11), ("PCA $(m=10)$", 10), ("PCA $(m=9)$", 9)]
    results, table = utilities.grid_search(logreg_callback, priors, l, dimred, dataset_types)
    np.savetxt(TABLES_OUTPUT_PATH + "logreg_results.csv", table, delimiter=";", fmt="%s", header=";".join(["Prior", "Lambda", "PCA", "Dataset", "MinDCF"]))

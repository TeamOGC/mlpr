...
class MVG(BaseClassifier):
    ...
    def fit(self, training_set: Tuple[npt.ArrayLike, npt.ArrayLike]):
        ...
        assert len(self.prior_probability) == len(
        ...
        MU_c = np.array([trn_data[:, trn_label == i].mean(1)
                        for i in classes]).T  # mu_i = mu_c[:, i]
        COV_c = np.array([cov(trn_data[:, trn_label == i])
                         for i in classes])  # COV_i = COV_c[i, :, :]
